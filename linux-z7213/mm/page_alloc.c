...
/*			if (migratetype == MIGRATE_CMA && strstr(current->comm, "memtester"))
				break;
*/
			area = &(zone->free_area[current_order]);
			if (list_empty(&area->free_list[migratetype]))
...
#ifdef CONFIG_CMA
	if (IS_ENABLED(CONFIG_CMA) && !strstr(current->comm, "memtester")) {
 ...
 * @next:	the minimal next pfn to try when alloc_contig_range fail.
 *
...
int alloc_contig_range(unsigned long start, unsigned long end,
		       unsigned migratetype, unsigned long *next)
{
	...
	if (test_pages_isolated(outer_start, end)) {
		pr_warn("alloc_contig_range test_pages_isolated(%lx, %lx) failed, start %lx, order %d\n",
		       outer_start, end, start, order);
		if (next)
			*next = outer_start + (1 << order);
		ret = -EBUSY;
		goto done;
	}
