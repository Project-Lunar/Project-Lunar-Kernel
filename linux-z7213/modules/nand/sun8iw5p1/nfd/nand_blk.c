...
static int do_blktrans_request(struct nand_blk_ops *tr,struct nand_blk_dev *dev,struct request *req)
{
	...
	/**
	 * do SEQ_FLUSH request
	 * userspace run sync/fsync will send this request
	 * nand driver uses write cache to improve write speed and life.
	 * we will flush write cache in every 500ms automatically,
	 * but when userspace calls sync/fsync, we should flush right now.
	 */
	if (req->cmd_flags & REQ_FLUSH) {
		nand_dev->flush_write_cache(nand_dev, 0xffff);
		goto request_exit;
	}
...
int nand_blk_register(struct nand_blk_ops *tr)
{
	...
	/* support SEQ_FLUSH for nand queue */
	blk_queue_flush(tr->rq, REQ_FLUSH);
