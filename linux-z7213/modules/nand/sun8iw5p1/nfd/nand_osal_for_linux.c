...
#define  NAND_DRV_DATE			0x20181107
...
//	{
	printk("NAND_DMASingleUnmap, dma addr not match! nand_index: 0x%x,
		tmp_dma_phy_addr:0x%x, mem_addr: 0x%x\n", nand_index,tmp_dma_phy_addr[nand_index], mem_addr);
//	mem_addr = tmp_dma_phy_addr[nand_index];
//	}
//
//	if(tmp_dma_len[nand_index] != len)
//	{
//		printk("NAND_DMASingleUnmap, dma len not match! nand_index: 0x%x,
			tmp_dma_len:0x%x, len: 0x%x\n", nand_index,tmp_dma_len[nand_index], len);
...
__u32 NAND_DMASingleUnmap(__u32 rw, __u32 buff_addr, __u32 len)
	...
	*/
	...
	if (rw == 1)
		dma_unmap_single(NULL, (dma_addr_t)mem_addr,
				len, DMA_TO_DEVICE);
	else
		dma_unmap_single(NULL, (dma_addr_t)mem_addr,
				len, DMA_FROM_DEVICE);
...
__s32 NAND_WaitDmaFinishWaitInterrupt(void)
{
	__u32 nand_index;
...
__s32 NAND_WaitDmaFinishNoInt(void)
{
...
{
	if (panic_flag == 0)
		NAND_WaitDmaFinishWaitInterrupt();
	else
		NAND_WaitDmaFinishNoInt();
}
...
__s32 NAND_WaitRbReadyWaitInterrupt(void)
{
	__u32 rb;
	__u32 nand_index;
...
__s32 NAND_WaitRbReadyNoInt(void)
{
    return 0;
}
...
{
	if (panic_flag == 0)
		NAND_WaitRbReadyWaitInterrupt();
	else
		NAND_WaitRbReadyNoInt();
}
...
#else
	if (nand_index == 0) {
		if ((__NAND_REG(NAND_CH0_INT_EN_REG) & NAND_RB_INT_BITMAP)
				&& (__NAND_REG(NAND_CH0_INT_ST_REG) & NAND_RB_INT_BITMAP))
			NAND_RbInterrupt();
	} else if (nand_index == 1) {
		if ((__NAND_REG(NAND_CH1_INT_EN_REG) & NAND_RB_INT_BITMAP)
...
#else
	if (nand_index == 0) {
		if ((__NAND_REG(NAND_CH0_INT_EN_REG) & NAND_DMA_INT_BITMAP)
				&& (__NAND_REG(NAND_CH0_INT_ST_REG) & NAND_DMA_INT_BITMAP))
			NAND_DMAInterrupt();
	} else if (nand_index == 1) {
		if ((__NAND_REG(NAND_CH1_INT_EN_REG) & NAND_DMA_INT_BITMAP)
				&& (__NAND_REG(NAND_CH1_INT_ST_REG) & NAND_DMA_INT_BITMAP))
			NAND_DMAInterrupt();
	}
...
void NAND_ShowEnv(__u32 type, char *name, __u32 len, __u32 *val)
{
	...
	if (type == 0) {
		...
		printk(KERN_ERR "kernel:NAND_ShowEnv, type error, %d!\n", type);
	}
