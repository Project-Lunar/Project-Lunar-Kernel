...
*Note         :
*****************************************************************************/
int lock_all_blk(void)
{
    struct _nftl_blk *nftl_blk = &(mytr.nftl_blk_head);

    while(nftl_blk->nftl_blk_next != NULL)
    {
        nftl_blk = nftl_blk->nftl_blk_next;
        mutex_lock(nftl_blk->blk_lock);
		printk("nand get lock\n");
    }
    return 0;
}
