...
{
	int ret;
	script_item_u   spinand_used_flag;
...
/*    char * dev_id = "nand_id";*/
	...
	type = script_get_item("spinand", "spinand_used", &spinand_used_flag);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		NAND_Print_DBG("spinand type err! %d", type);
	}
	nand_dbg_err("nand init start, spinand_used_flag is %d\n", spinand_used_flag.val);

	nand_cache_level.val = 0;
	type = script_get_item("spinand", "spinand_cache_level", &nand_cache_level);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		NAND_Print_DBG("spinand_cache_level err! %d", type);
		nand_cache_level.val = 0;
	}

	nand_capacity_level.val = 0;
	type = script_get_item("spinand", "spinand_capacity_level", &nand_capacity_level);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		NAND_Print_DBG("spinand_capacity_level err! %d\n", type);
		nand_capacity_level.val = 0;
	}

	init_completion(&spinand_dma_done);

	if (spinand_used_flag.val == 0) {
		kfree(data);
		nand_dbg_err("spinand driver is disabled \n");
		return 0;
	}
...
/*	if((0 != storage_type)&&(1 != storage_type)&&(2 != storage_type))*/
	if (5 == storage_type) {
...
#endif
		test_mbr(data);
		ret = nand_info_init(p_nand_info, 0, 20, data);
	}
	kfree(data);
	if (ret != 0) {
		return ret;
...
void __exit nand_exit(void)
{
	...
	type = script_get_item("spinand", "spinand_used", &spinand_used_flag);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
		nand_dbg_err("spinand type err!");
	nand_dbg_err("spinand_used_flag is %d\n", spinand_used_flag.val);

	if (spinand_used_flag.val == 0) {
		nand_dbg_err("spinand driver is disabled \n");
	}
	nand_dbg_err("spinand driver unregister start \n");
	platform_driver_unregister(&nand_driver);
	nand_dbg_err("spinand driver unregister end \n");

	nand_dbg_err("spinand device unregister start \n");
	platform_device_unregister(&nand_device);
	nand_dbg_err("spinand device unregister end \n");
	exit_blklayer();
